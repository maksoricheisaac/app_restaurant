...
const keyFor = (slug: string) => `flashmenu_cart_${slug}`;
...
/**
 * Panier client persisté dans localStorage (par slug) : survit à la navigation
 * vers le checkout et au refresh. Une ligne = un plat + une combinaison
 * d'options ; ré-ajouter la même combinaison incrémente la quantité.
 */
export function useMenuCart(slug: string) {
