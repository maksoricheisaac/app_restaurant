...
test.describe('QR Flow — Menu public client', () => {
  test('la carte publique s\'affiche', async ({ page }) => {
...
test.describe('QR Flow — Order tracking', () => {
  test('page de tracking avec UUID inconnu — pas de crash', async ({ page }) => {
    ...
    await page.goto(`/menu/track/${fakeUuid}`);
...
test.describe('QR Flow — Public menu API', () => {
  test('GET /public-menu — carte publique servie', async ({ request }) => {
    ...
    expect(body.restaurant).toBeTruthy();
    expect(Array.isArray(body.menu)).toBe(true);
  });

  test('POST /public-menu/order — items vides → 400', async ({ request }) => {
    const API_BASE = process.env.E2E_API_URL || 'http://localhost:3000/api/v1';
    const res = await request.post(`${API_BASE}/public-menu/order`, {
      data: { type: 'dine_in', items: [] },
    });
    // 400 (items vides) ou 403 (session de menu absente) — jamais 500
