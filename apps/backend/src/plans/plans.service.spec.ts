...
describe('PlanLimitService', () => {
  ...
  describe('assertMenuItemLimit', () => {
    ...
    it('throws ForbiddenException when count >= limit (FREE plan)', async () => {
      ...
      await expect(service.assertMenuItemLimit('t1')).rejects.toThrow(ForbiddenException);
    });
    ...
    it('never throws for ENTERPRISE plan (unlimited)', async () => {
      prisma.tenant.findUnique.mockResolvedValue({ plan: 'enterprise', status: 'active' });
    ...
    it('treats suspended tenant as free plan', async () => {
      prisma.tenant.findUnique.mockResolvedValue({ plan: 'pro', status: 'suspended' });
  ...
  describe('assertTableLimit', () => {
    it('blocks creation at FREE limit (3 tables)', async () => {
      ...
      await expect(service.assertTableLimit('t1')).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('assertStaffMemberLimit', () => {
    it('blocks new staff member when FREE limit reached', async () => {
      ...
      await expect(service.assertStaffMemberLimit('t1')).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('assertMonthlyOrderLimit', () => {
    it('blocks order at FREE monthly limit (10)', async () => {
      ...
      await expect(service.assertMonthlyOrderLimit('t1')).rejects.toThrow(ForbiddenException);
    });
    ...
    it('allows order under FREE monthly limit', async () => {
      ...
      await expect(service.assertMonthlyOrderLimit('t1')).resolves.toBeUndefined();
    });
