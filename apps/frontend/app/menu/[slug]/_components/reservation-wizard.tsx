...
export function ReservationWizard({
  ...
  if (done) {
    return (
      <SuccessScreen slug={slug} restaurant={restaurant} color={color} onBrand={onBrand} prettyDate={prettyDate} guests={guests} time={time} />
    );
  }
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-md px-5 pb-10 pt-8">
        {step === 0 ? (
          <a href={`/menu/${slug}`} className="mb-6 inline-flex items-center gap-2 text-sm font-medium text-white/85 transition-colors hover:text-white">
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-6" style={{ backgroundColor: WARM.page }}>
      <div className="w-full max-w-sm space-y-4 rounded-3xl p-8 text-center shadow-lg" style={{ backgroundColor: WARM.card, border: `1px solid ${WARM.border}` }}>
        ...
        <a href={`/menu/${slug}`} className="inline-block rounded-xl px-6 py-3 text-sm font-bold" style={{ backgroundColor: color, color: onBrand }}>
          Retour au menu
        </a>
      </div>
    </div>
