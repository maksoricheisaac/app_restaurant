...
const addExceptionalClosure = async (_: { date: Date }) =>
  ({ success: false, message: 'Fonctionnalité non disponible pour le moment.' }) as const;
const getExceptionalClosures = async (): Promise<Array<{ id: string; date: Date; reason?: string | null }>> => [];
const deleteExceptionalClosure = async (_: string) =>
  ({ success: false, message: 'Fonctionnalité non disponible pour le moment.' }) as const;
...
export function OpeningHoursForm() {
  ...
  const { data: hoursData, isLoading } = useOpeningHours();
  const updateMutation = useUpdateOpeningHours();

  useEffect(() => {
    if (hoursData) {
      form.reset({ openingHours: hoursData });
    }
  }, [hoursData, form]);
