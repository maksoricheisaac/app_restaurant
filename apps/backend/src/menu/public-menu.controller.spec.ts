...
const mockMenuSessionService = { generate: jest.fn().mockReturnValue('test-token') };
...
describe('PublicMenuController', () => {
  ...
  describe('createOrder', () => {
    it('delegates entirely to PublicOrderService', async () => {
      ...
      const result = await controller.createOrder('le-maquis', dto, 'test-token');
    ...
    it('propagates errors from PublicOrderService', async () => {
      ...
      await expect(
        controller.createOrder('unknown', { type: 'dine_in', items: [] } as any, undefined),
