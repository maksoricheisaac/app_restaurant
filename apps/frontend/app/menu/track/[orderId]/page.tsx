...
interface TrackingData {
  ...
  restaurant: { name: string; logo: string | null; primaryColor: string | null; currency: string } | null;
...
export default function TrackOrderPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  useEffect(() => {
    ...
    socket.on('status-update', (payload: { status: OrderStatus }) => {
      ...
      updateStoredStatus(orderId, payload.status);
    });
  ...
  if (isError || !tracking)
    return (
      <StatusBlock
        ...
        action={
          <a href={'/menu'} className="text-sm font-semibold transition-colors" style={{ color: WARM.muted }}>
            Retour au menu
          </a>
        }
      />
  ...
  const color = normalizeHex(tracking.restaurant?.primaryColor ?? null);
  const onBrand = readableOn(color);
  const currency = tracking.restaurant?.currency ?? 'XAF';
    ...
    <div className="flex min-h-screen flex-col" style={{ backgroundColor: WARM.page }}>
      ...
      <header className="sticky top-0 z-10 shadow-sm" style={{ backgroundColor: WARM.card, borderBottom: `1px solid ${WARM.border}` }}>
        <div className="mx-auto flex max-w-md items-center gap-3 px-4 py-3.5">
          <a href={'/menu'} className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl transition-colors" style={{ backgroundColor: WARM.surfaceAlt }} aria-label="Retour au menu">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2.5">
            {tracking.restaurant?.logo ? (
              <div className="relative h-8 w-8 flex-shrink-0 overflow-hidden rounded-lg">
                <Image src={tracking.restaurant?.logo} alt={tracking.restaurant?.name ?? 'Le restaurant'} fill className="object-cover" sizes="32px" />
              </div>
            ) : (
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-bold leading-none" style={{ color: WARM.ink }}>{tracking.restaurant?.name ?? 'Le restaurant'}</p>
      ...
      <main className="mx-auto w-full max-w-md flex-1 space-y-4 px-4 py-6">
        {isCancelled ? (
          <div className="flex flex-col items-center gap-4 rounded-3xl border border-red-100 bg-white p-8 text-center shadow-sm">
            ...
            <a href={'/menu'} className="mt-1 inline-flex items-center gap-2 rounded-xl px-6 py-3 text-sm font-bold text-white transition-colors" style={{ backgroundColor: WARM.ink }}>
              Nouvelle commande
            </a>
          </div>
        ) : (
