import { Order, OrderType, DashboardOrderStatus } from '@/types/order';
...
interface OrderCardDetailedProps {
  order: Order;
  updateOrderMutation: UseMutationResult<unknown, Error, { id: string; status: DashboardOrderStatus; tableId?: string | null }>;
  handlePrintPDF: (order: Order) => Promise<void>;
  statusColors: Record<DashboardOrderStatus, string>;
  statusLabels: Record<DashboardOrderStatus, string>;
  typeLabels: Record<OrderType, string>;
  formatCurrency: (amount: number) => string;
}

export const OrderCardDetailed: React.FC<OrderCardDetailedProps> = ({
  order,
  updateOrderMutation,
  handlePrintPDF,
  statusColors,
  ...
  const handleStatusChange = (orderId: string, newStatus: DashboardOrderStatus) => {
      ...
      <CardHeader className="pb-2 flex flex-row items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <Badge className={statusColors[order.status as DashboardOrderStatus]}>
              {statusLabels[order.status as DashboardOrderStatus]}
            </Badge>
        ...
        <div className="flex flex-col gap-2 w-full mt-4">
          <div className="flex flex-col md:flex-row gap-2 w-full">
            <Select
              ...
              onValueChange={(value) => {
                if (order.status !== value) {
                  try {
                    handleStatusChange(order.id, value as DashboardOrderStatus);
                  } catch (error) {
            ...
            <Button
              ...
              disabled={!canCancel || updateOrderMutation.isPending}
            >
              Annuler
            </Button>
          </div>
