...
export function MenuFooter({ tenant, color }: { tenant: Tenant; color: string }) {
    ...
    <footer className="mt-4 text-white" style={{ backgroundColor: WARM.dark }}>
      <div className="mx-auto max-w-2xl space-y-8 px-5 pb-8 pt-12">
        <div className="flex items-start gap-4">
          {tenant.logo ? (
            <Image src={tenant.logo} alt={tenant.name} width={64} height={64} className="h-16 w-16 flex-shrink-0 rounded-2xl object-cover shadow-lg" />
          ) : (
          ...
          <div className="space-y-1.5">
            <h3 className="font-display text-2xl leading-tight text-white">{tenant.name}</h3>
            {tenant.cuisineType && (
        ...
        <div className="flex items-center justify-between border-t border-white/10 pt-5">
          <p className="text-xs text-white/35">© {new Date().getFullYear()} {tenant.name}</p>
