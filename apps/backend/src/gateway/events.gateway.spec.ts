...
describe('EventsGateway', () => {
  ...
  describe('handleConnection', () => {
    it('sets client.data.user from a valid Authorization header token', async () => {
      ...
      mockJwtService.verify.mockReturnValue({
        sub: 'u1', email: 'a@b.com', role: 'owner', tenantId: 'tenant-1',
      });

      await gateway.handleConnection(client);

      expect(client.data.user).toEqual({ id: 'u1', email: 'a@b.com', role: 'owner', tenantId: 'tenant-1' });
    });

    it('sets client.data.user from auth.token', async () => {
      const client = buildSocket({ authToken: 'auth.token.here' });
      mockJwtService.verify.mockReturnValue({
        sub: 'u2', email: 'b@c.com', role: 'manager', tenantId: 'tenant-2',
      });
    ...
    it('prefers Authorization header over auth.token', async () => {
      const client = buildSocket({ authHeader: 'Bearer header.token', authToken: 'auth.token' });
      mockJwtService.verify.mockReturnValue({ sub: 'u1', email: 'a@b.com', role: 'owner', tenantId: null });
    ...
    it('leaves client.data.user undefined for invalid token (silent fail)', async () => {
      ...
      mockJwtService.verify.mockImplementation(() => { throw new Error('Invalid JWT'); });
  ...
  describe('handleJoinTenant', () => {
    it('returns Unauthorized when client is not authenticated', async () => {
      ...
      const result = await gateway.handleJoinTenant(client, { tenantId: 'tenant-1' });
    ...
    it('joins the correct room when membership exists', async () => {
      const client = buildSocket({ userData: { id: 'u1', tenantId: 'tenant-1' } });
      prisma.tenantMembership.findUnique.mockResolvedValue({ id: 'm1', role: 'owner' });
    ...
    it('verifies membership against the correct userId and tenantId', async () => {
      const client = buildSocket({ userData: { id: 'user-uuid', tenantId: 'tenant-uuid' } });
      ...
      expect(prisma.tenantMembership.findUnique).toHaveBeenCalledWith({
        where: { userId_tenantId: { userId: 'user-uuid', tenantId: 'tenant-uuid' } },
      });
    });
  });
  ...
  describe('handleJoinOrder', () => {
    ...
    it('returns error for missing or non-string orderId', async () => {
      ...
      expect(await gateway.handleJoinOrder(client, { orderId: null as any })).toEqual({
        status: 'error', message: 'Invalid orderId',
      });
    });
    ...
    it('returns error when order not found', async () => {
      ...
      const result = await gateway.handleJoinOrder(client, { orderId: 'ghost-order' });
    ...
    it('blocks authenticated user from joining order of a different tenant', async () => {
      const client = buildSocket({ userData: { id: 'u1', tenantId: 'tenant-X' } });
      prisma.order.findUnique.mockResolvedValue({ id: 'o1', tenantId: 'tenant-Y' });
    ...
    it('allows authenticated user from the same tenant to join', async () => {
      ...
      const result = await gateway.handleJoinOrder(client, { orderId: 'order-1' });
    ...
    it('checks membership for authenticated user without tenantId claim', async () => {
      const client = buildSocket({ userData: { id: 'u1', tenantId: undefined } });
      prisma.order.findUnique.mockResolvedValue(order);
