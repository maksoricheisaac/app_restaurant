...
import type { NavItem, NavGroup, NavCollapsible } from './types'
        ...
        <DropdownMenuContent side='right' align='start' sideOffset={4}>
          ...
          {item.items.map((sub: NavItem) => (
            <DropdownMenuItem key={`${sub.title}-${sub.url}`} asChild>
              <Link
                href={sub.url}
                className={`${checkIsActive(href, sub) ? 'bg-orange-600 text-white"' : 'text-gray-700 hover:bg-orange-100 hover:text-orange-700'}`}
              >
                {sub.icon && <sub.icon />}
