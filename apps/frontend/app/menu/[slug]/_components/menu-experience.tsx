...
export function MenuExperience({
  ...
  const { tenant, menu } = initial;
  const color = normalizeHex(tenant.primaryColor);
  ...
  const goToCheckout = () => {
    router.push(`/menu/${slug}/order${tableId ? `?table=${tableId}` : ''}`);
  };
    ...
    <div className="min-h-screen font-sans" style={{ backgroundColor: WARM.page, color: WARM.ink }}>
      <div ref={searchRef}>
        <MenuHero
          ...
          reservationHref={`/menu/${slug}/reservation`}
          searchQuery={search}
      ...
      <MenuFooter tenant={tenant} color={color} />
    </div>
  );
}
